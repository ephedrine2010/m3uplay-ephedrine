...
- **Trends rails scroll horizontally, the pane does not.** Each source band is an `overflow-x:auto` flex row built by `js/trends_board/rail.js`, which adds ‹ › nav buttons, wheel→horizontal panning (it stops swallowing the wheel at either end so the board can still scroll vertically) and mouse drag-to-pan. This only works because `#content`, `#trends-board` and `.tb-board` all carry **`min-width: 0`** — grid/flex items default to `min-width:auto` (min-content), which would widen the pane to fit all 20 cards instead of letting the rail scroll. Don't remove those.
